...
export default function RightTop() {
  ...
  return (
    <div className="relative h-full w-full rounded-3xl bg-[#363C43] px-10 py-5 shadow-[6px_6px_0px_0px_rgba(0,0,0,0.2)]">
      <div className="mx-auto ml-3 flex flex-row justify-between gap-2 rounded-2xl bg-neutral-950 p-2">
        {links.map((link, index) => (
          <button
            key={index}
            onClick={() => setActiveIndex(index)}
            className={`relative flex-1 overflow-hidden rounded-xl px-6 py-3 text-center text-[12px] transition-all duration-700 ease-in-out md:text-sm ${
              activeIndex === index
                ? "bg-neutral-800 text-white shadow-lg"
                : "group cursor-pointer text-neutral-400/80"
            }`}
          >
            <span className="absolute inset-0 -translate-x-full rounded-2xl bg-linear-to-r from-[#0e0e0e] to-[#252a30] transition-transform duration-700 ease-in-out group-hover:translate-x-0"></span>
            <span className="relative z-10 font-medium">{link}</span>
          </button>
        ))}
      </div>
      <div className="font-jakarta mx-auto mt-7 p-2 text-justify text-sm sm:text-[16px] font-light text-[#969696]">
        ...
        between 9-10 AM. This is a great time to reach me! In my free time, I
        love to ..
      </div>
      <svg
        width="24"
